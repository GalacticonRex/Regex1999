	...
	static void array_printatoms( array* arr ) {
		...
		for ( i=0;i<arr->size/sizeof(atom*);++i ) {
			...
			if ( a->dealloc == destroy_literal ) {
				...
				printf( "FINISH   " );
			} else if ( a->dealloc == destroy_line_limit ) {
				if ( ((atom_line_limits*) a->data)->type == ATOM_LINE_START )
					printf( "LNSTART  " );
				else if ( ((atom_line_limits*) a->data)->type == ATOM_LINE_END )
					printf( "LNEND    " );
			}
		} printf( "}\n" );
		fflush( NULL );
...
#ifdef __DEBUG__
	uint32 limit = 29;
	while ( *str != '\0' && iter < limit && this != re->initial->next ) {
