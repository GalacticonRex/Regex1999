...
static void apply_new_layer( atom* init ) {
	
}
static void undo_old_layer( atom* init ) {
...
static atom* regex_reset( regex* re, const char* val ) {
	re->strstart = val;
	uint32 i = 0;
	uint32 sz = array_sizeptr( re->working );
	printf( "%d -- %d\n", sz, re->working->size );
	array_print( re->working );
	for ( ;i<sz;++i ) {
		array* quan = (array*) array_getptr( re->working, i );
		printf( "%p\n", quan );
		fflush( NULL );
		array_clear32( quan, 0 );
	}
	...
	while ( *str != '\0' && iter < 80 && this->data != NULL ) {
		...
		if ( this == NULL ) { // match failed
			if ( array_sizeptr( re->track_atom ) != 0 ) {
				...
				for ( ;begin<=end;++begin ) {
					array* quan = (array*) array_getptr( re->working, begin );
					uint32* ptr = (uint32*) array_top32( quan, 0 );
					(*ptr) = 0;
				}
